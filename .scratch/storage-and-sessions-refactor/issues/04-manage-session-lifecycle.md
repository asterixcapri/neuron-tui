...
- [ ] `start()` mints a distinct storage-safe key and returns its empty History; a Session remains absent from `list()` until its History is non-empty.
...
- [ ] A new `Sessions` instance over the same storage can list and resume earlier Sessions because the module maintains its own index in the `sessions` namespace.
