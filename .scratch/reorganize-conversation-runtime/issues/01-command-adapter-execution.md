...
- [x] CommandAdapterInterface replaces CommandControlsInterface. It contains
...
- [x] Commands retains ordered mounting, exact identifier lookup, and
...
- [x] An unknown identifier reaches afterExecution() with an unknown
  CommandExecution, without calling admission or invoking a Command. run()
  returns the Adapter's output unchanged.
- [x] For a known identifier, admission receives the first matching Command.
  ...
  is introduced.
- [x] An admitted Command runs with the supplied Adapter and unchanged
...
- [x] The exception guard surrounds the Command invocation. Exceptions from
...
- [x] CommandExecution remains a technical completed/unknown/failed outcome,
...
- [x] Migrate every shared Command and existing test Adapter to the new
...
- [x] The backend example admits its Commands, collects notices, warnings,
...
- [x] The backend example and its coverage demonstrate two independent
  ...
  history entries.
- [x] The Host Application continues to supply Agent execution. No HTTP
...
- [x] Update shared usage documentation to describe the one-call lifecycle,
...
- [x] Full neuron-interaction tests and static analysis pass. Record the
...
Implemented in neuron-interaction at `d823bce842c16d69d9432d4b7ea98ca21d618021`
on `feat/command-adapter-execution`, companion PR
https://github.com/asterixcapri/neuron-interaction/pull/2. The source package
replaces the controls interface, owns the complete Commands lifecycle, and
migrates its Commands, concrete test Adapters, backend example, and docs.

Verification against that revision: `composer test` passed (118 tests,
355 assertions); `composer stan` passed at maximum level;
`php examples/backend.php` demonstrated selection and continuation;
`git diff --check` passed. TUI dependency integration is recorded in ticket 02.
