...
final class ResponseInterruptionTest extends TestCase
{
    ...
    public function testProviderFailureAfterEscapeRetainsTheInterruptedTurnAndAdvancesOnce(bool $partial, bool $queued): void
    {
        ...
        $expected = $partial ? ['First question', 'Partial.'] : [];
        ...
        self::assertSame($partial, str_contains($display, 'Turn interrupted.'));
        ...
        if ($queued) {
            self::assertSame(array_slice($messages, 0, $partial ? 3 : 1), $provider->getRecorded()[1]->messages);
            self::assertSame(array_slice($messages, 0, $partial ? 5 : 3), $provider->getRecorded()[2]->messages);
        }
    }
    ...
    public function testPickerConsumesFirstEscapeBeforeTurnInterruption(): void
    {
        ...
        self::assertStringNotContainsString('Stop requested', $afterFirstEscape);
        $messages = $agent->getChatHistory()->getMessages();
    ...
    public function testEscapeRetainsPartialTextAdvancesFifoAndPreservesDraftEditing(): void
    {
        ...
        self::assertStringContainsString('Stop requested', $pending);
        $display = AnsiUtils::stripAnsiCodes($terminal->getOutput());
    ...
    public function testEscapeBeforeFirstTextWaitsAndRetainsTheFirstChunk(): void
    {
